...
MAX_DISPLAYED_EXPLOITS = 6

SCRIPT_DRIFT_PATTERN = re.compile(
...
def has_script_drift(text):
    """
    Detect generated text containing a non-Latin script the model was
    never asked to write in. Observed live: a 7B model, mid-Executive-
    Summary generation, produced a coherent English opening then drifted
    into garbled CJK-script text for the remainder of the section - not
    a RAG/data problem (the vector store was audited character-by-
    character and contains zero non-Latin content), purely model-side
    non-determinism, likely correlated with longer RAG-augmented prompts
    increasing variance. A report in the wrong script is unusable
    regardless of whether the underlying facts are grounded, so this is
    checked independently of - and before - the CVE/port/vuln-class
    grounding checks, since grounding-checking garbled text is meaningless.
    """
    return bool(SCRIPT_DRIFT_PATTERN.search(text))


def format_exploit_titles(exploits):
    """
    Render an exploit list capped at MAX_DISPLAYED_EXPLOITS titles, with a
    '+N more (see session.json)' suffix for the rest. Applied both to the
    Findings Reference Table and to the per-finding facts sent to the
    model - an uncapped list (observed: 40+ exploits on a single VNC
    finding) bloats both the rendered table and the prompt context for no
    benefit; the full list always remains available in session.json.
    """
    if not exploits:
        return "-"
    titles = [e.get("title", "?") for e in exploits if e.get("title")]
    if not titles:
        return "-"
    shown = titles[:MAX_DISPLAYED_EXPLOITS]
    remaining = len(titles) - len(shown)
    result = ", ".join(shown)
    if remaining > 0:
        result += f", +{remaining} more (see session.json)"
    return result


def _finding_signature(finding):
    """Content-based signature for a finding: exact set of CVE IDs +
    exact set of exploit identifiers (EDB-ID preferred, falling back to
    title). Two findings sharing this signature exactly are the same
    underlying vulnerability surfaced on a different port (e.g. Samba's
    139+445 pair, 8 identical RPC ports) - merging by content equality
    means two genuinely distinct findings can never be accidentally
    combined just because they happen to share a service name."""
    cve_ids = tuple(sorted(c["id"] for c in finding.get("cves", [])))
    exploit_keys = tuple(sorted(
        e.get("edb_id") or e.get("title") or "" for e in finding.get("exploits", [])
    ))
    return (cve_ids, exploit_keys)


def group_duplicate_findings(findings):
    """
    Merge findings that share an identical CVE+exploit signature into one
    logical finding covering multiple ports/protocols. Findings with an
    empty signature (no CVEs and no exploits) are never merged - grouping
    'nothing found' services together by that empty signature would
    incorrectly imply they're the same finding.
    """
    groups = {}
    order = []
    for f in findings:
        sig = _finding_signature(f)
        key = (id(f),) if sig == ((), ()) else sig
    ...
    for key in order:
        ...
        if len(group) > 1:
            primary["_grouped_endpoints"] = [
                f"{f['port']}/{f['proto']}" for f in group[1:]
            ]
        merged.append(primary)
    return merged


def format_finding_port_label(finding):
    """Consistent 'PORT/PROTO (+N identical endpoint(s): ...)' label used
    across the facts table, technical findings header, and recommendations
    header - built from the _grouped_endpoints group_duplicate_findings()
    attaches to a merged finding."""
    base = f"{finding['port']}/{finding['proto']}"
    extra = finding.get("_grouped_endpoints")
    if extra:
        base += f" (+{len(extra)} identical endpoint(s): {', '.join(extra)})"
    return base


def top_finding_identifiers(with_evidence):
    """
    Deterministically identify the TRUE single highest-CVSS finding across
    all evidence-bearing findings - ground truth independent of anything
    the model writes, used to verify (and if necessary correct) the
    Executive Summary's claim about the most critical issue. A single-call
    Executive Summary has no structural guarantee of picking the real top
    finding the way the per-finding technical/recommendations sections do,
    so this closes that gap with verify-retry-correct instead of trusting
    the model to get it right from the ranked list alone.
    """
    best = None
    best_score = -1.0
...
def build_top_findings_summary(with_evidence, max_items=5):
    """
    Deterministic (Python-ranked, not model-generated) top-N findings by
    CVSS, injected into the Executive Summary prompt so the model has an
    explicit, correct ranking handed to it. Reduces but does not guarantee
    the model actually leads with it - see exec_summary_names_top_finding()
    for the verification/correction layer that closes that gap.
    """
    scored = []
    for f in with_evidence:
        max_cvss = finding_max_cvss(f)
        if max_cvss > 0:
            scored.append((max_cvss, f))
    scored.sort(key=lambda x: x[0], reverse=True)

    lines = ["Findings ranked by highest CVSS score (most critical first):"]
    for score, f in scored[:max_items]:
        top_cve = next(
            (c for c in f.get("cves", []) if c.get("cvss_score") == score), None
        )
        cve_id = top_cve["id"] if top_cve else "?"
        lines.append(
            f"  CVSS {score}: {format_finding_port_label(f)} {f['service']} - {cve_id}"
        )
    return "\n".join(lines)


def exec_summary_names_top_finding(text, top_finding):
    """Check whether generated Executive Summary text names the true top
    finding's CVE ID or port - the narrow, cheap check backing the
    retry-once-then-correct pattern. Not a full coherence check - script
    drift is checked separately by has_script_drift()."""
    if not top_finding:
        return True  # nothing to check against (no CVSS-scored findings)
    return (top_finding["cve_id"] in text) or (str(top_finding["port"]) in text)
...
def build_single_finding_facts(finding):
    ...
    lines = [f"{format_finding_port_label(finding)} {finding['service']} - {finding['version_raw']}"]
    for cve in finding.get("cves", []):
        ...
        if desc:
            line += f": {desc}"
        lines.append(line)
    exploit_titles = [e.get("title") for e in finding.get("exploits", []) if e.get("title")]
    for title in exploit_titles[:MAX_DISPLAYED_EXPLOITS]:
        lines.append(f"Exploit available: {title}")
    remaining = len(exploit_titles) - MAX_DISPLAYED_EXPLOITS
    if remaining > 0:
        lines.append(f"(+{remaining} more exploit(s), see session.json)")
    return "\n".join(lines)
...
def _generate_and_check_finding_text(prompt_text, model, url, temperature, timeout,
    ...
    if has_script_drift(text):
        warn = (f"Section '{section_title}' ({label}) generated non-Latin script "
                f"text (language drift) - retrying once.")
        eprint(f"[!] {warn}")
        retry_messages = messages + [
            {"role": "assistant", "content": text},
            {"role": "user", "content": "Your previous response contained text in "
             "the wrong language/script. Rewrite your entire response in English "
             "only, keeping the same facts and meaning."},
        ]
        try:
            text = call_ollama(retry_messages, model=model, url=url, temperature=temperature,
                                timeout=timeout, max_tokens=max_tokens)
        except RuntimeError as e:
            text = f"[Generation failed on script-drift retry: {e}]"
        if has_script_drift(text):
            warn2 = (f"Section '{section_title}' ({label}) still contained non-Latin "
                     f"script after retry - replacing with a plain fallback line.")
            eprint(f"[!] {warn2}")
            warnings.append(warn2)
            text = (f"Automated narrative generation for this finding produced "
                    f"unreadable output after two attempts. See the Findings "
                    f"Reference Table above for the CVE/exploit facts for this "
                    f"service; this line was inserted deterministically in place "
                    f"of the failed generation.")
        else:
...
def generate_technical_findings_per_finding(with_evidence, model, url, temperature,
    ...
    for finding in with_evidence:
        ...
        label = f"{format_finding_port_label(finding)} {finding['service']}"
        ...
        header = f"### {format_finding_port_label(finding)} {finding['service']} - {finding['version_raw']}"
...
def generate_recommendations_per_finding(with_evidence, model, url, temperature,
    ...
    for finding in with_evidence:
        ...
        header = f"**{format_finding_port_label(finding)} {finding['service']}" + \
...
def render_facts_table(session):
    lines = ["## Findings Reference Table", "",
             ...
             "|---|---|---|---|---|"]
    grouped = group_duplicate_findings(session.get("cve_exploit_findings", []))
    for f in grouped:
        cve_str = ", ".join(c["id"] for c in f.get("cves", [])) or "-"
        exp_str = format_exploit_titles(f.get("exploits", []))
        lines.append(
            f"| {format_finding_port_label(f)} | {f['service']} | {f['version_raw']} | "
            f"{cve_str} | {exp_str} |"
        )
    if not grouped:
        lines.append("| - | - | - | - | - |")
...
def generate_report(session, model=DEFAULT_MODEL, url=OLLAMA_DEFAULT_URL,
    ...
    # Group duplicate-signature findings (e.g. Samba's 139+445 pair, or a
    # handful of identical RPC ports) into one logical finding up front -
    # every downstream use (technical findings, recommendations, and the
    # facts table via render_facts_table) sees the same grouped view, so
    # a service never shows up once collapsed in the table but repeated
    # in the narrative sections or vice versa.
    grouped_findings = group_duplicate_findings(session.get("cve_exploit_findings", []))
    session = dict(session, cve_exploit_findings=grouped_findings)
    ...
    for key in sections:
        ...
        # plus a deterministic CVSS-ranked top-findings summary, so the
        # model has an explicit correct ranking handed to it - backed up
        # by exec_summary_names_top_finding()'s verify-retry-correct
        # pattern below, since handing a ranking to the model does not
        # guarantee it actually leads with it.
        top_finding = top_finding_identifiers(with_evidence)
        top_summary = build_top_findings_summary(with_evidence)
        user_prompt = prompt_fn(f"{facts}\n\n{top_summary}", rag_chunks)
        ...
        # Script-drift check FIRST - a garbled non-Latin-script response
        # makes every downstream check (top-finding naming, grounding)
        # meaningless, so this must be resolved before anything else.
        if has_script_drift(text):
            warn = f"Section '{title}' generated non-Latin script text (language drift) - retrying once."
            eprint(f"[!] {warn}")
            retry_messages = messages + [
                {"role": "assistant", "content": text},
                {"role": "user", "content": "Your previous response contained text in "
            ...
            try:
                ...
                text = f"[Generation failed on script-drift retry: {e}]"
            if has_script_drift(text):
                warn2 = f"Section '{title}' still contained non-Latin script after retry - replacing with a plain fallback summary."
                eprint(f"[!] {warn2}")
                warnings.append(warn2)
                if top_finding:
                    text = (f"Automated summary generation produced unreadable output "
                            f"after two attempts. The most critical issue identified in "
                            f"this assessment is {top_finding['cve_id']} on "
                            f"{top_finding['port']}/{top_finding['proto']} "
                            f"({top_finding['service']}), CVSS {top_finding['cvss_score']}. "
                            f"See the Findings Reference Table below for full details.")
                else:
                    text = ("Automated summary generation produced unreadable output "
                            "after two attempts. See the Findings Reference Table below "
                            "for the full facts of this assessment.")
            else:
                warnings.append(warn)

        # Verify the summary actually leads with the TRUE highest-CVSS
        # finding - a single-call section has no structural guarantee of
        # this the way the per-finding sections do (observed: with
        # IDENTICAL session.json input, the model named the true top
        # finding in some runs and a lower-severity one in others).
        if not exec_summary_names_top_finding(text, top_finding):
            warn = (f"Executive Summary did not name the actual highest-severity "
                    f"finding ({top_finding['port']}/{top_finding['proto']} "
                    f"{top_finding['service']}, {top_finding['cve_id']}, CVSS "
                    f"{top_finding['cvss_score']}) - retrying once with a more "
                    f"explicit prompt.")
            eprint(f"[!] {warn}")
            warnings.append(warn)
            forceful_prompt = (
                f"{user_prompt}\n\nIMPORTANT: Your summary MUST explicitly name "
                f"{top_finding['cve_id']} on port {top_finding['port']} as the single "
                f"most critical finding - this is the true highest-CVSS issue "
                f"(CVSS {top_finding['cvss_score']}) and must be identified as such."
            )
            retry_messages = [
                {"role": "system", "content": SYSTEM_PROMPT},
                {"role": "user", "content": forceful_prompt},
            ]
            try:
                retry_text = call_ollama(retry_messages, model=model, url=url, temperature=temperature,
                                          timeout=timeout, max_tokens=max_tokens)
                retry_text = strip_duplicate_heading(retry_text, title)
            except RuntimeError as e:
                retry_text = f"[Generation failed: {e}]"

            if has_script_drift(retry_text):
                eprint(f"[!] Retry also produced non-Latin script - keeping original text and appending a deterministic correction instead.")
                text = text.rstrip() + (
                    f"\n\n> **Correction:** The single most critical finding in this "
                    f"assessment is {top_finding['cve_id']} on "
                    f"{top_finding['port']}/{top_finding['proto']} "
                    f"({top_finding['service']}), CVSS {top_finding['cvss_score']}."
                )
            elif exec_summary_names_top_finding(retry_text, top_finding):
                print("[*] Retry succeeded - executive summary now names the correct top finding.")
                text = retry_text
            else:
                warn2 = "Executive Summary still did not name the correct top finding after retry - appended a deterministic correction."
                eprint(f"[!] {warn2}")
                warnings.append(warn2)
                text = retry_text.rstrip() + (
                    f"\n\n> **Correction:** The single most critical finding in this "
                    f"assessment is {top_finding['cve_id']} on "
                    f"{top_finding['port']}/{top_finding['proto']} "
                    f"({top_finding['service']}), CVSS {top_finding['cvss_score']}."
